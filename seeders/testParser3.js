...
function parseHTMLUsers() {
  ...
  while (i < matches.length) {
    ...
    // FIRST: Check for section markers (before parsing entries)
    if (text.includes('Lecturers')) {
      inLecturersSection = true;
      inStudentsSection = false;
      console.log(`📌 Lecturers section found at index ${i}: "${text}"`);
      i++;
      continue;
    }
    ...
    if (text.includes('Students')) {
      ...
      console.log(`📌 Students section found at index ${i}: "${text}"`);
      i++;
      continue;
    }
    
    // THEN: Parse lecturer entries
...
console.log('🧪 Testing Fixed HTML Parser...\n');
parseHTMLUsers();
console.log('\n✅ Test complete!');
